...
每次研究必须覆盖以下平台类别，根据研究主题的语言和领域选择具体平台：

#### 中文平台（中文主题必搜，英文主题按需）

| 平台 | 搜索方式 | 适合内容 |
|------|---------|---------|
| 知乎 | `site:zhihu.com {关键词}` | 深度讨论、专业观点、用户体验 |
| CSDN | `site:csdn.net {关键词}` | 技术文章、开发者视角 |
| 掘金 | `site:juejin.cn {关键词}` | 前端/开发技术、工具评测 |
| 微博 | `site:weibo.com {关键词}` | 实时热点、公众人物观点 |
| 36氪 | `site:36kr.com {关键词}` | 商业分析、创投动态 |
...
| 平台 | 搜索方式 | 适合内容 |
|------|---------|---------|
| Reddit | `site:reddit.com {keywords}` | 社区讨论、真实用户反馈 |
| Twitter/X | `site:x.com OR site:twitter.com {keywords}` | 实时动态、行业人物观点 |
| GitHub | `site:github.com {keywords}` | 开源项目、技术实现、Issues讨论 |
| Hacker News | `site:news.ycombinator.com {keywords}` | 技术社区深度讨论 |
| Medium | `site:medium.com {keywords}` | 深度文章、行业分析 |
| Dev.to | `site:dev.to {keywords}` | 开发者社区、技术教程 |
...
| 权威媒体 | TechCrunch、The Verge、Wired、机器之心、量子位等 |
| 学术/报告 | arXiv、行业报告、研究机构发布 |
...
第二组：平台定向搜索（社区/社交媒体）
{N+1}. site:zhihu.com {关键词} — 知乎讨论
{N+2}. site:reddit.com {keywords} — Reddit讨论
{N+3}. site:x.com {keywords} — Twitter/X动态
...
...
第一轮广度搜索完成：执行 {N} 次查询，收集 {M} 条结果
  - 中文平台：{X} 条（知乎 {a}、CSDN {b}、掘金 {c}...）
  - 英文平台：{Y} 条（Reddit {d}、Twitter {e}、GitHub {f}...）
  - 通用来源：{Z} 条
...
### 英文社区与媒体
5. [{文章标题}]({URL}) — Reddit r/{subreddit}，{日期}
6. [{文章标题}]({URL}) — Twitter/X @{username}，{日期}
7. [{文章标题}]({URL}) — TechCrunch，{日期}

### 技术与学术
8. [{文章标题}]({URL}) — GitHub，{日期}
9. [{文章标题}]({URL}) — arXiv，{日期}
```
