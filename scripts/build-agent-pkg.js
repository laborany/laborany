...
function main() {
  ...
  const runLocalBinScript = join(rootDir, 'scripts', 'run-local-bin.js')
  ...
  try {
    ...
    run(process.execPath, [runLocalBinScript, 'pkg', 'dist/bundle.cjs', '--targets', target, '--output', output, '--config', 'pkg.json'], agentDir)
