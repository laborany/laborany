...
- 微信 Bot 远程触发（通过微信官方 ClawBot / iLink 私聊执行任务）
...
最新补充：微信 Bot 已完成接入并通过真实微信 E2E，发版说明见 [docs/release-notes-v0.4.5-wechat-bot.md](docs/release-notes-v0.4.5-wechat-bot.md)。
...
└──────────────────────────────────────────────────────────────────┘
        ...
        ┌──────────────┐
        │  微信客户端    │──── HTTPS / iLink ─→ agent-service (WeChat Bot)
        └──────────────┘
```
...
- WeChat Bot：微信远程触发入口，通过官方 ClawBot / iLink 协议接收私聊消息，复用 converse + execute 链路
...
通过微信官方 ClawBot / iLink 协议把 LaborAny 直接接入微信私聊，不依赖 OpenClaw Gateway；支持在设置页扫码绑定账号，也支持用环境变量手工覆盖 `WECHAT_BOT_TOKEN`。

能力概览：

- 智能路由 — 复用 converse 意图分发，自动匹配最合适的技能
- 流式消息 — 使用多条消息回传分析和执行结果
- 文件支持 — 支持图片、文件输入，以及任务产物回传到微信
- 定时任务 — 支持自然语言或 `/cron` 命令创建 `cron / at / every` 三类调度
- 命令系统 — `/skill`、`/skills`、`/cron`、`/new`、`/stop`、`/help`、`/model`、`/home`

真实验证状态：

- 已完成真实微信扫码绑定与私聊 E2E
- 文本、追问续跑、`/new`、`/stop`、`/cron once` 已验证通过
- 图片输入、文件输入、图片回传、文件回传已验证通过
- 当前微信能力闭环与 QQ / 飞书远程 Bot 对齐

推荐接入流程：

1. 在设置页开启 `微信 Bot`，保存 `WECHAT_ENABLED`、白名单和基础地址配置。
2. 点击“扫码绑定”，使用已开通 ClawBot 的微信扫码完成授权。
3. 绑定成功后启动 Bot，在微信私聊发送 `/help` 验证连接。
...
# WECHAT_BOT_TOKEN=your-wechat-bot-token   # 可选，优先级高于扫码保存的本地凭据
WECHAT_ALLOW_USERS=user1@im.wechat,user2@im.wechat
WECHAT_REQUIRE_ALLOWLIST=false
WECHAT_BOT_NAME=LaborAny
WECHAT_DEFAULT_SKILL=__generic__
WECHAT_POLL_TIMEOUT_MS=35000
WECHAT_TEXT_CHUNK_LIMIT=1000
```

> 详细规范见 [docs/wechat-bot-spec.md](docs/wechat-bot-spec.md)，发版说明见 [docs/release-notes-v0.4.5-wechat-bot.md](docs/release-notes-v0.4.5-wechat-bot.md)。
...
特性：动态定时器（事件驱动，精确唤醒）、手动触发、执行历史、通知推送。定时任务既可在桌面端创建，也可在飞书、QQ、微信 Bot 中通过自然语言或 `/cron` 命令创建；远程 Bot 创建的任务默认会把结果推送回原私聊通道。
