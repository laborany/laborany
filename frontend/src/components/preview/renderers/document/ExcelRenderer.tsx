...
export function ExcelRenderer({ artifact }: RendererProps) {
  ...
  if (error || sheets.length === 0) return <ErrorState name={artifact.name} error={error} url={artifact.url} />

  const current = sheets[activeSheet]

  return (
    <div className="flex h-full flex-col">
      {sheets.length > 1 && <SheetTabs sheets={sheets} active={activeSheet} onSelect={setActiveSheet} />}
      <TableContent sheet={current} />
      <StatusBar rows={current?.data.length || 0} cols={current?.data[0]?.length || 0} />
    </div>
  )
}
...
async function parseExcel(buffer: ArrayBuffer): Promise<XLSX.WorkBook | null> {
  try {
    ...
    return XLSX.read(recompressed, { type: 'array' })
  }
}
...
function SheetTabs({ sheets, active, onSelect }: { sheets: ExcelSheet[]; active: number; onSelect: (i: number) => void }) {
  return (
      ...
      {sheets.map((sheet, i) => (
        <button
          ...
          className={`shrink-0 rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
            i === active ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-accent/50 hover:text-foreground'
          }`}
        >
          {sheet.name}
        </button>
...
function TableContent({ sheet }: { sheet: ExcelSheet }) {
  if (!sheet || sheet.data.length === 0) {
    return <div className="flex h-full items-center justify-center text-sm text-muted-foreground">空表格</div>
  }
    ...
    <div className="flex-1 overflow-auto bg-background">
      <table className="w-full border-collapse text-sm">
        <thead className="sticky top-0 z-10 bg-muted">
          <tr>
            <th className="sticky left-0 z-20 w-10 border border-border bg-muted px-2 py-2 text-center text-xs font-medium text-muted-foreground">#</th>
            {sheet.data[0].map((cell, i) => (
              <th key={i} className="min-w-[100px] border border-border px-3 py-2 text-left font-medium text-foreground">{cell}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {sheet.data.slice(1).map((row, rowIdx) => (
            <tr key={rowIdx} className="hover:bg-muted/50">
              <td className="sticky left-0 border border-border bg-muted/50 px-2 py-2 text-center text-xs text-muted-foreground">{rowIdx + 2}</td>
              {row.map((cell, cellIdx) => (
                <td key={cellIdx} className="border border-border px-3 py-2 text-foreground">{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
function StatusBar({ rows, cols }: { rows: number; cols: number }) {
  return (
