import type { PipelineContext, StepResult } from './types.js'

export function createPipelineContext(input: Record<string, unknown>): PipelineContext {
  return {
    input,
    steps: [],
    current: 0,
    files: [],
  }
}

export function addPipelineStepResult(context: PipelineContext, result: StepResult): PipelineContext {
  return {
    ...context,
    steps: [...context.steps, result],
    current: context.current + 1,
    files: [...context.files, ...result.files],
  }
}

export function buildPipelineStepPrompt(
  stepPrompt: string,
  context: PipelineContext,
  stepIndex: number,
  totalSteps: number,
): string {
  const renderedPrompt = renderPrompt(stepPrompt, context)
  const contextSummary = buildContextSummary(context, stepIndex, totalSteps)
  return `${contextSummary}\n\n---\n\n${renderedPrompt}`
}

function renderPrompt(prompt: string, context: PipelineContext): string {
  return prompt.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_match, expression) => {
    const expr = String(expression).trim()
    if (expr === 'prev.output') {
      const prev = context.steps[context.steps.length - 1]
      return prev?.output || ''
    }
    if (expr === 'prev.files') {
      const prev = context.steps[context.steps.length - 1]
      return prev?.files.join(', ') || ''
    }
    if (expr === 'files') {
      return context.files.join(', ')
    }

    const value = getNestedValue(
      {
        input: context.input,
        steps: context.steps,
        files: context.files,
      },
      expr,
    )

    if (value === null || value === undefined) return ''
    if (typeof value === 'object') return JSON.stringify(value)
    return String(value)
  })
}

function getNestedValue(obj: Record<string, unknown>, path: string): unknown {
  const parts = path.split(/\.|\[|\]/).filter(Boolean)
  let current: unknown = obj

  for (const part of parts) {
    if (current === null || current === undefined) {
      return undefined
    }
    if (typeof current !== 'object') {
      return undefined
    }
    current = (current as Record<string, unknown>)[part]
  }

  return current
}

function buildContextSummary(
  context: PipelineContext,
  stepIndex: number,
  totalSteps: number,
): string {
  const lines: string[] = [
    '## 复合 Skill 执行上下文',
    '',
    `当前步骤：${stepIndex + 1} / ${totalSteps}`,
  ]

  if (Object.keys(context.input).length > 0) {
    lines.push('', '### 输入参数')
    for (const [key, value] of Object.entries(context.input)) {
      lines.push(`- ${key}: ${JSON.stringify(value)}`)
    }
  }

  if (context.steps.length > 0) {
    lines.push('', '### 前序步骤结果')
    for (const step of context.steps) {
      const statusIcon = step.status === 'completed' ? '✅' : '❌'
      lines.push('', `**步骤 ${step.stepIndex + 1}** [${statusIcon}]`)
      const outputSummary = step.output.length > 500 ? `${step.output.slice(0, 500)}...` : step.output
      lines.push(outputSummary)
      if (step.files.length > 0) {
        lines.push('', `生成文件: ${step.files.join(', ')}`)
      }
    }
  }

  return lines.join('\n')
}
