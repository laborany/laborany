...
function resolveVideoGenMcpServerPath(): string {
  ...
  const bundled = join(MODULE_DIR, '..', 'video-gen', 'mcp', 'mcp-server.mjs')
  if (existsSync(bundled)) return bundled

  const sidecar = join(dirname(process.execPath), 'video-gen', 'mcp', 'mcp-server.mjs')
  if (existsSync(sidecar)) return sidecar

  throw new Error(`laborany-video-gen MCP server not found under ${MODULE_DIR}`)
}
