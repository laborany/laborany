...
async function discoverWebSocketPath(port) {
  return new Promise((resolve) => {
...
function startHeartbeat() {
  ...
  heartbeatTimer = setInterval(() => {
    if (!ws || (ws.readyState !== WS.OPEN && ws.readyState !== 1)) return;
    sendCDP('Browser.getVersion').catch(() => {
      // ignore; close handler will reset state if socket is gone
    });
