...
function initTables(db: Database.Database): void {
  ...
  try {
    db.exec(`ALTER TABLE cron_jobs ADD COLUMN source_channel TEXT DEFAULT 'desktop'`)
  } catch { /* 列已存在 */ }
  try {
    db.exec(`ALTER TABLE cron_jobs ADD COLUMN source_feishu_open_id TEXT`)
  } catch { /* 列已存在 */ }
  try {
  ...
  try {
    db.exec(`ALTER TABLE cron_jobs ADD COLUMN source_wechat_user_id TEXT`)
  } catch { /* 列已存在 */ }
  try {
  ...
  try {
    db.exec(`ALTER TABLE cron_jobs ADD COLUMN notify_wechat_user_id TEXT`)
  } catch { /* 列已存在 */ }
  ...
  try {
    db.exec(`CREATE INDEX IF NOT EXISTS idx_jobs_source_wechat_user_id ON cron_jobs(source_wechat_user_id)`)
  } catch { /* 索引已存在 */ }
}
...
export function listJobsBySourceOpenId(openId: string, sourceChannel = 'feishu'): CronJob[] {
  const normalizedChannel = sourceChannel === 'qq'
    ? 'qq'
    : sourceChannel === 'wechat'
      ? 'wechat'
      : 'feishu'
  const sourceColumn = normalizedChannel === 'qq'
    ? 'source_qq_open_id'
    : normalizedChannel === 'wechat'
      ? 'source_wechat_user_id'
      : 'source_feishu_open_id'
...
export function createJob(req: CreateJobRequest): CronJob {
  ...
  const sourceChannel = req.source?.channel ?? 'desktop'
  const sourceFeishuOpenId = sourceChannel === 'feishu'
    ? (req.source?.feishuOpenId?.trim() || null)
    : null
  ...
  const sourceQqOpenId = sourceChannel === 'qq'
    ...
    : null
  const sourceWechatUserId = sourceChannel === 'wechat'
    ? (req.source?.wechatUserId?.trim() || null)
    : null
  ...
  const notifyQqOpenId = notifyChannel === 'qq_dm'
    ...
    : null
  const notifyWechatUserId = notifyChannel === 'wechat_dm'
    ? (req.notify?.wechatUserId?.trim() || req.source?.wechatUserId?.trim() || null)
    : null
  ...
  getDb().prepare(`
    INSERT INTO cron_jobs (
      ...
      source_channel, source_feishu_open_id, source_feishu_chat_id, source_qq_open_id, source_wechat_user_id,
      notify_channel, notify_feishu_open_id, notify_qq_open_id, notify_wechat_user_id,
...
export function updateJob(id: string, req: UpdateJobRequest): CronJob | null {
  ...
  if (req.notify !== undefined) {
    ...
    updates.push('notify_channel = ?', 'notify_feishu_open_id = ?', 'notify_qq_open_id = ?', 'notify_wechat_user_id = ?')
    values.push(
      notifyChannel,
      notifyChannel === 'feishu_dm' ? (req.notify.feishuOpenId?.trim() || null) : null,
      notifyChannel === 'qq_dm' ? (req.notify.qqOpenId?.trim() || null) : null,
      notifyChannel === 'wechat_dm' ? (req.notify.wechatUserId?.trim() || null) : null,
    )
  }
...
function rowToJob(row: unknown): CronJob {
  ...
  return {
    ...
    sourceWechatUserId: r.source_wechat_user_id as string | undefined,
    ...
    notifyWechatUserId: r.notify_wechat_user_id as string | undefined,
